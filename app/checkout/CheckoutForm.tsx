  ...
  if (hydrated && items.length === 0) {
    return (
      <div className="bg-white border border-[#f0f0f0] p-10 text-center">
        ...
        <a href="/" className="inline-block px-8 py-3.5 text-xs tracking-[0.2em] uppercase bg-[#111] text-white">
          Browse the edit
        </a>
      </div>
    );
    ...
    <form action={formAction} className="grid lg:grid-cols-[1fr_320px] gap-10 items-start" noValidate>
      ...
      <div className="bg-white border border-[#f0f0f0] p-6 space-y-5">
        ...
        {[
          ...
          return (
            <div key={f.id}>
              <label htmlFor={f.id} className="block text-[0.65rem] tracking-[0.2em] uppercase text-[#111] mb-1.5">
                {f.label}
              </label>
              <input
                id={f.id}
              ...
              />
              {err && <p id={`${f.id}-err`} role="alert" className="text-red-500 text-xs mt-1">{err}</p>}
            </div>
          );
        })}
        ...
        {state.status === "error" && state.message && (
          <p role="alert" className="text-red-500 text-sm">{state.message}</p>
        )}
