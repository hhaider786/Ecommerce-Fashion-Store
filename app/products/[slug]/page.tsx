      ...
      <main id="main" className="pt-24 pb-20">
        <nav className="max-w-7xl mx-auto px-6 mb-6" aria-label="Breadcrumb">
          <ol className="flex items-center gap-2 text-[0.65rem] tracking-[0.2em] uppercase text-[#666]">
            ...
            <li><Link href={`/category/${product.category.toLowerCase()}`} className="hover:text-[#111]">{product.category}</Link></li>
            <li><ChevronRight size={11} aria-hidden /></li>
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-10 lg:gap-16">
          <div className="space-y-3">
            <div className="relative aspect-[3/4] bg-[#f5f5f5] overflow-hidden">
              <ImageWithBlur
                ...
                style={{ viewTransitionName: `product-${product.slug}` } as React.CSSProperties}
              />
            </div>
          ...
          <div className="lg:sticky lg:top-28 lg:self-start">
            ...
            <h1
              className="text-3xl md:text-4xl font-light text-[#111] mb-3"
              ...
              <div className="flex items-center gap-1" aria-label={`Rated ${product.rating} out of 5`}>
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    ...
                    size={13}
                    aria-hidden
                    fill={i < Math.round(product.rating) ? "#111" : "none"}
                    className="text-[#111]"
                  />
                ))}
            ...
            <ul className="mt-10 space-y-3 border-t border-[#f0f0f0] pt-6">
              <li className="flex items-center gap-3 text-sm text-[#444]">
                <Truck size={16} className="text-[#111]" aria-hidden />
                <span>Free shipping on orders over £100</span>
              </li>
              <li className="flex items-center gap-3 text-sm text-[#444]">
                <RotateCcw size={16} className="text-[#111]" aria-hidden />
                <span>30-day returns, on us</span>
              </li>
              <li className="flex items-center gap-3 text-sm text-[#444]">
                <ShieldCheck size={16} className="text-[#111]" aria-hidden />
                <span>Two-year quality guarantee</span>
              </li>
            </ul>
        ...
        {related.length > 0 && (
          <section className="max-w-7xl mx-auto px-6 mt-24">
            ...
            <h2
              className="text-3xl font-light text-[#111] mb-8"
              style={{ fontFamily: "var(--font-cormorant-var), Georgia, serif" }}
            >
      ...
      <JsonLd
        ...
        data={[
          ...
          breadcrumbSchema([
            ...
            { name: product.category, url: `${SITE_URL}/category/${product.category.toLowerCase()}` },
