...
export function CartView() {
  ...
  if (!hydrated) {
    return <div className="py-20 text-center text-[#888]">Loading…</div>;
  }

  if (items.length === 0) {
    return (
      <div className="border border-[#f0f0f0] py-24 text-center">
        <ShoppingBag size={42} className="mx-auto text-[#ddd] mb-4" aria-hidden />
        <p className="text-[#666] mb-6">Your bag is empty.</p>
        <Link href="/" className="inline-block px-8 py-3.5 text-xs tracking-[0.2em] uppercase bg-[#111] text-white hover:bg-[#333] transition-colors">
          Continue shopping
        </Link>
      </div>
    );
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-10 lg:gap-16">
      <ul className="lg:col-span-2 divide-y divide-[#f0f0f0]">
        {items.map((item) => (
          <li key={`${item.id}-${item.size}`} className="flex gap-5 py-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[0.65rem] tracking-wider text-[#666] uppercase mt-1">
              ...
              <div className="flex items-center justify-between mt-5">
                <div className="flex items-center border border-[#e0e0e0]" role="group" aria-label={`Quantity for ${item.name}`}>
                  <button
                    ...
                    aria-label="Decrease"
                    className="px-3 py-2 text-[#555] hover:text-[#111]"
                  >
      ...
      <aside className="border border-[#f0f0f0] p-6 h-fit lg:sticky lg:top-28">
        ...
        <Link href="/" className="block text-center mt-3 text-xs tracking-wider underline text-[#666] hover:text-[#111]">
          Continue shopping
        </Link>
      </aside>
    </div>
