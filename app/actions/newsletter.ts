...
export async function subscribeNewsletter(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  const limit = rateLimit(`newsletter:${getClientKey(h)}`, 5, 60_000);
  if (!limit.ok) {
    return { status: "error", message: `Too many requests. Try again in ${limit.retryAfter}s.` };
  }
  ...
  await logSubmission("newsletter", { email: result.data.email });
