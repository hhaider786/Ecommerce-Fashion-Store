      ...
      >
        <Tilt3D max={5} className="relative overflow-hidden bg-[#f5f5f5] mb-3 aspect-[3/4]">
          <ImageWithBlur
            ...
            style={{ opacity: hovered ? 0 : 1, viewTransitionName: hovered ? undefined : `product-${product.slug}` } as React.CSSProperties}
          />
          <ImageWithBlur
          ...
          <button
            ...
            className="absolute top-3 right-3 w-8 h-8 bg-white/90 flex items-center justify-center opacity-0 group-hover:opacity-100 focus-visible:opacity-100 transition-opacity duration-200 hover:bg-white z-10"
          >
          ...
          <AnimatePresence>
            {hovered && (
              ...
              >
                <div
                  className="bg-white px-3 pt-3 pb-1 flex gap-1.5 justify-center flex-wrap"
                  role="radiogroup"
...
type FilterPillProps = { active: boolean; onClick: () => void; children: ReactNode };
...
export default function ProductGrid() {
  ...
  const setFilter = useCallback((patch: Partial<FilterState>) => {
    ...
    startTransition(() => {
      router.replace(`/?${params.toString()}#products`, { scroll: false });
    });
  }, [filters, router]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <AnimatePresence>
          {filtersOpen && (
            ...
            >
              <fieldset>
                ...
                <div className="flex flex-wrap gap-2">
                  {["All", "Women", "Men", "Accessories"].map((c) => (
                    <FilterPill key={c} active={filters.category === c} onClick={() => setFilter({ category: c })}>
                      {c}
                    </FilterPill>
                  ))}
                </div>
              ...
              <div>
                <label htmlFor="max-price" className="section-label mb-3 block">
              ...
              <fieldset>
                ...
                <div className="flex flex-wrap gap-2">
                  {["All", "New", "Bestseller", "Sale"].map((b) => (
                    <FilterPill key={b} active={filters.badge === b} onClick={() => setFilter({ badge: b })}>
                      {b}
                    </FilterPill>
                  ))}
                </div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 text-[#666]">
            <p className="text-lg" style={{ fontFamily: "var(--font-cormorant-var), Georgia, serif" }}>
