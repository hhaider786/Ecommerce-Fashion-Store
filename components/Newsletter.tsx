    ...
    <section className="bg-[#111] py-20 px-6">
      <div className="max-w-2xl mx-auto text-center">
        ...
        >
          ...
          {state.status === "success" ? (
            ...
            <form action={formAction} className="flex flex-col sm:flex-row gap-0 max-w-md mx-auto" noValidate>
              ...
              <input
                ...
                className="flex-1 bg-white/5 border border-white/15 text-white placeholder-[#666] px-4 py-3.5 text-sm focus:outline-none focus:border-white/50 transition-colors"
              />
              <button
                type="submit"
                disabled={pending}
                className="bg-white text-[#111] px-6 py-3.5 text-xs tracking-[0.2em] uppercase font-medium hover:bg-[#f0f0f0] transition-colors disabled:opacity-60"
              >
          ...
          {error && (
            <p id="newsletter-error" role="alert" className="text-red-300 text-xs mt-3">
              {error}
            </p>
          )}
